...
class DevpiTestCase(unittest.TestCase):
    payload = {
        ...
        "PLUGIN_SERVER": os.getenv("TEST_DEVPI_SERVER", "http://127.0.0.1:3141"),
    ...
    def _wait_for_devpi_to_start(cls, env, clientdir):
        ...
        while retries_left > 0:
            try:
                run_devpi.select_server(vargs["server"], clientdir=clientdir)
            except SystemExit:
                retries_left -= 1
                time.sleep(1)
                continue
    ...
    def _ensure_test_index_exists(self, env, clientdir):
        ...
        run_devpi.select_server(t_vargs["server"], clientdir=clientdir)
        run_devpi.login(
            t_vargs["username"], t_vargs["password"], clientdir=self.default_clientdir
        )
        try:
            run_devpi.create_index(t_vargs["index"], clientdir=clientdir)
        except SystemExit:
            pass
    ...
    def test_upload(self):
        ...
        self._ensure_test_index_exists(self.payload, self.default_clientdir)
        ...
        package_path = os.path.join(self.payload["DRONE_REPO_NAME"])
